...
This package provide a tidy API to graph/network manipulation. While network 
...
tables and provide `dplyr` verbs for manipulating them. Furthermore it provides
...
The goal is to support all verbs from dplyr that makes sense, which is almost
all of them. It is definetly easier to list the ones that wont get supported and
describe why:
...
merged. If data summaries are needed these can be achieved by extracting the 
...
modify the data in ways that does not make sense in a graph context. The 
solution is again to extract the data prior to the `do` call.

The current status is that everything else, except for the joins and set
methods are supported.

### 2. Provide Constructors for all general relational data structures
The goal is that any data structure seems relational should be able to be fed
into `as_tbl_graph` - this entails conversion functions into `igraph,` which is
the underlying data structure that powers `tidygraph`. Currently the following 
is supported:
...
automatically update the edges to fit, are to combine parallel edges. Another
...
All algorithms where it makes sense should get a wrapper where it is not 
necessary to specify the graph object and which nodes or edges are getting 
